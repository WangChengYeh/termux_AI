# Termux Fork: SDK Upgrade + Python Preload Proposal

This document proposes forking `termux/termux-app`, upgrading it to the latest Android SDK/AGP toolchain, and provisioning a Python-ready bootstrap so first-run includes a working `python` and optional preselected pip wheels offline.

## Goals
- Deliver a maintained fork that:
  - Targets the latest stable Android SDK (compile/target SDK) and modern AGP/Gradle.
  - Keeps Termux functionality intact with scoped storage and up-to-date permissions.
  - Preloads the `python` package in the Termux bootstrap to avoid large first-run installs.
  - Optionally pre-installs a curated set of Python wheels offline for instant use.
...
1. Fork via GitHub UI (or CLI) to org/user of choice.
   - CLI example (requires `gh`):
     ...
     cd termux-app
     git checkout -b feat/sdk-upgrade-python-preload
     ```
2. Keep upstream in sync:
   ...
   ```
3. CI will build APKs and attach to releases; bootstrap artifacts will be built by a companion workflow (see “Bootstrap with Python”).
...
## Bootstrap with Python (Preload)
We want first-run bootstrap to include the Termux `python` package without user interaction.
...
Optional: Preload pip wheels offline
...
### Python for MCP CLI
Goal: Make the Model Context Protocol CLI (`mcp`) usable immediately after first run.

- Base packages in bootstrap: add `python`, `openssl`, `libffi` to ensure SSL modules work.
...
- Verification (first run smoke test):
  ...
  - If using Codex CLI as a server or client, document configuration under `$HOME/.config/mcp/*`.

### Additional Preload: Codex CLI (aarch64)
...
### App Changes for Custom Bootstrap
...
- Strategy:
  - Embed minimal bootstrap + Python + MCP wheels + Codex (aarch64) under `app/src/main/assets`.
...
- GitHub Actions matrix for bootstrap (optional separate repo or same):
  - Use Docker on Ubuntu runners to build bootstraps for all ABIs with `python`.
  - Attach `bootstrap-*.zip` to a release and publish a JSON index the app can consume.
...
- E2E manual: Fresh install on emulators for API 24/29/33/35; verify bootstrap download, extraction, and `python --version` works immediately.

## Risks & Mitigations
- Bootstrap size growth: host artifacts off-APK; compress and prune extras.
- License obligations: audit Python & wheels; include LICENSE files in `assets/NOTICE` and release notes.
- ABI compatibility: ensure `python` is built for each ABI; avoid shipping incompatible wheels.
...
## Deliverables
- GitHub fork with branch `feat/sdk-upgrade-python-preload`.
- Upgraded Gradle/AGP config and manifest fixes.
- App code changes to use custom bootstrap endpoint.
- CI workflows for APK and bootstrap artifacts.
- A release with APK and `bootstrap-*.zip`.
...
5. Wire app to use custom bootstrap index/URLs.
...
# Build bootstraps (in termux-packages)
export TERMUX_BOOTSTRAP_PACKAGES="bash coreutils ca-certificates python"
./scripts/run-docker.sh ./build-package.sh -a aarch64 bootstrap
```
